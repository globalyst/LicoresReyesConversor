...
public class XmlLoader {
	...
	public Document loadFile(String filename){
		...
		try {
	    	...
	    	System.out.println(TAG + "No se puede encontrar el archivo  " + filename);
	    }
		return result;
	  }
	...
	public boolean loadObjects(String filename) {
		...
		for (int i = 0; i < objectList.getLength(); i++) {
			...
			if (objectNode.getNodeType() == Node.ELEMENT_NODE) {
				 ...
				 String aux = eElement.getAttribute("aux");
				 String accountFilter = eElement.getAttribute("filter");
				 boolean canCreate = Boolean.parseBoolean(eElement.getAttribute("cancreate"));
				 XmlObject xmlObject = new XmlObject(objectId,model,company,aux,accountFilter,canCreate);
	...
	public XmlObject getObjectByName(String name) {
		XmlObject result = null;
		for (int i = 0;i < objectList.size(); i++) {
			if (objectList.get(i).getId().equals(name) ) {
